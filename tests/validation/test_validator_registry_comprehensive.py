"""Comprehensive tests for ValidatorRegistry class."""
...
class InvalidValidator:
    """Invalid validator that doesn't inherit from BaseValidator."""
...
class TestValidatorRegistryContractCheckingIntegration:
    """Test ValidatorRegistry contract checking integration with loading."""
    ...
    def test_user_validator_with_violations_is_skipped(self):
        """Test that user validators with contract violations are skipped."""
        # Create a validator with violations
        validator_code = '''
from lamia.validation.base import BaseValidator, ValidationResult

class ViolatingValidator(BaseValidator):
    @property
    def name(self) -> str:
        return 123  # Violation: should return string
    ...
    async def validate(self, response: str, execution_context=None, **kwargs) -> ValidationResult:
        return ValidationResult(is_valid=True)
'''
        validator_path = os.path.join(self.validators_dir, "violating.py")
        with open(validator_path, 'w') as f:
            f.write(validator_code)
        ...
        try:
            ...
            # Violating validator should not be loaded
            assert "violating" not in registry._user_validators
        finally:
            os.chdir(original_cwd)
...
class TestValidatorRegistryEdgeCases:
    ...
    def test_check_validator_with_none(self):
        """Test check_validator handles None gracefully."""
        registry = ValidatorRegistry(extensions_folder="test_extensions")

        # Should handle None without crashing
        try:
            passed, violations = registry.check_validator(None)
        except (AttributeError, TypeError):
            # Expected to fail, but shouldn't crash the entire system
            pass
...
class TestValidatorRegistryIntegration:
    ...
    def test_full_workflow_built_in_validator(self):
        """Test full workflow of getting and using a built-in validator."""
        registry = ValidatorRegistry(extensions_folder="test_extensions")

        # Get a built-in validator
        if registry._built_in_validators:
            validator_name = list(registry._built_in_validators.keys())[0]
            ...
            # Verify it passes contract checks
            passed, violations = registry.check_validator(validator_class)
            assert passed is True

    def test_registry_maintains_separation_of_concerns(self):
        """Test that built-in and user validators are kept separate."""
        registry = ValidatorRegistry(extensions_folder="test_extensions")

        # Manually add a user validator
        registry._user_validators["user_test"] = ValidTestValidator

        # Built-in validators should be unchanged
        assert "user_test" not in registry._built_in_validators

        # User validators should only have the one we added
        assert "user_test" in registry._user_validators

        # Built-in validators should not be in user validators
        for built_in_name in registry._built_in_validators.keys():
            assert built_in_name not in registry._user_validators or built_in_name == "user_test"
