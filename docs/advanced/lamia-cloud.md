...
`.env` files are never copied into the deployed container, so a script that reads a key from the environment finds nothing in the cloud until you name that key. List the ones your script needs under `cloud.secrets`:
...
Leave the values where they already are — your project's `.env` — and deploy:

```bash
lamia my_script.lm --remote
```

Each listed key is written to Secret Manager, and the Cloud Run Job is given a *reference* to it rather than the value. GCP injects it as an environment variable when the job runs, so `os.environ["THIRD_PARTY_API_KEY"]` works exactly as it does locally, and the value never appears in the job's configuration.

Nothing outside that list leaves your machine. Model provider keys get no special treatment — an unlisted key is simply missing at runtime, whatever it is. Note that you should not add API keys for built-in models to the cloud.secrets list, as they authenticate through Vertex AI.

### Cleanup

Removing a schedule or trigger deletes the secrets it was using:
...
A secret shared by several deployments survives until the last one referencing it is gone, so removing one of two schedules that use the same key leaves the key in place.

Editing a value in `.env` does not reach a running deployment on its own — deploy again to push the new value.
