...
- **Check logs**: Use `lamia schedule list` to get the job id, then run `lamia schedule logs <id>`. Local jobs write to `~/.lamia/logs/schedules/<id>/schedule.log`; cloud jobs fetch from Cloud Logging.
